...
const LearningOutcomesSection = ({ formState, formHandlers }: LearningOutcomesSectionType) => {
  ...
  return (
    <Paper elevation={0} variant="outlined" sx={{ p: 3 }}>
      <Typography variant="h6" gutterBottom>
        {t('program.learningOutcomes')}
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        {t('program.learningOutcomesSubtitle')}
      </Typography>
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 2 }}>
          <TextField
            label={t('program.objective')}
            value={objectiveItem}
            onChange={(e) => setObjectiveItem(e.target.value)}
            helperText={t('program.objectiveHelperText')}
            required
            fullWidth
          />
          <Button
